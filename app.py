...
import os
import zipfile
from imgOVDoc import roi_matching
from imgOVDoc import roi_matching_ov
...
def upload():
    if request.method == "POST":
        ...
        if request.form["model"] == "1":
            result = 'Mobilenet' + str(roi_matching.run_match(os.path.join(img_dir, file2.filename)))
        else:
            result = 'Openvino' + str(roi_matching_ov.roi_match())
